    ...
    def forward(
        ...
        **kwargs,
        ...
        if not decode:
            ...
            num_random_activate_tokens = int(self.random_sparse * (q_len-self.window_size))
            ...
            if num_activate_tokens > 0 and num_activate_tokens < (q_len - self.window_size) :
                ...
                if num_random_activate_tokens > 0:
                    for h in range(self.num_heads):
                        rand_permute = torch.randperm(self.indices_to_remove.shape[-1]).to(self.indices_to_remove.device)
                        self.indices_to_remove[:,h] = self.indices_to_remove[:,h, :, rand_permute]
                    
                    self.indices_to_remove = self.indices_to_remove[...,:-num_random_activate_tokens]
